...
主程序入口位于 `Boston Big Guy/USER/main.c`，初始化顺序如下：
...
- 工程文件：`Boston Big Guy/USER/Boston Big Guy.uvprojx`
- 编译器：`ARM Compiler 5`（工程中 `uAC6=0`）
- 固件库：`STM32F10x Standard Peripheral Library`
- Device Pack：`Keil.STM32F1xx_DFP.2.4.1`
- 启动文件：`Boston Big Guy/CORE/startup_stm32f10x_hd.s`

## 工程结构

- `Boston Big Guy/USER`
  - 主程序入口、动作逻辑、Keil 工程文件、中断与系统配置
- `Boston Big Guy/HARDWARE`
  - 舵机、推进电机、MS5837、AS5600、JY901S、A4988 等驱动
- `Boston Big Guy/BSP`
  - GPIO、定时器、软件 I2C、Flash 等板级支持代码
- `Boston Big Guy/SYSTEM`
  - 串口、延时、系统时钟等系统层代码
- `Boston Big Guy/STM32F10x_FWLib`
  - STM32F10x 标准外设库
- `Boston Big Guy/OBJ`
  - Keil 编译输出目录
...
以下分布基于 `Boston Big Guy/BSP/GPIO.c`、`Boston Big Guy/BSP/timer.c`、`Boston Big Guy/HARDWARE/Servo.c`、`Boston Big Guy/HARDWARE/A4988.c`、`Boston Big Guy/SYSTEM/usart/usart.c`。
...
动作函数集中定义在 `Boston Big Guy/USER/Action.c`。

### 2. 串口数据帧

USART2 接收使用中断方式，关键字节定义在 `Boston Big Guy/SYSTEM/usart/usart.h`：
...
基础运动控制来自 `Boston Big Guy/USER/main.h`：
...
1. 安装 `Keil MDK5` 与 `STM32F1xx_DFP` 设备包。
2. 打开工程 `Boston Big Guy/USER/Boston Big Guy.uvprojx`。
3. 选择目标后编译，输出目标名为 `TIMER`。
4. 连接下载器后烧录到 `STM32F103C8`。

如果只想快速定位核心文件，优先看这些：

- `Boston Big Guy/USER/main.c`
- `Boston Big Guy/USER/Action.c`
- `Boston Big Guy/BSP/GPIO.c`
- `Boston Big Guy/BSP/timer.c`
- `Boston Big Guy/SYSTEM/usart/usart.c`
