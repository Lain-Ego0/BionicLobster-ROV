...
主程序入口位于 `firmware/USER/main.c`，初始化顺序如下：
...
- 开发链：`Linux + Arm GNU Toolchain + CMake + OpenOCD`
- 下载器：`DAPLink (CMSIS-DAP)`
- 固件库：`STM32F10x Standard Peripheral Library`
- GNU 启动文件：`firmware/CORE/startup_stm32f103xb_gcc.c`
...
以下分布基于 `firmware/BSP/GPIO.c`、`firmware/BSP/timer.c`、`firmware/HARDWARE/Servo.c`、`firmware/HARDWARE/A4988.c`、`firmware/SYSTEM/usart/usart.c`。
...
- 新增 GCC 启动入口：`firmware/CORE/startup_stm32f103xb_gcc.c`
...
- 将 `SYSTEM/sys/sys.c` 中编译器专用的 `MSR_MSP()` 写法改为 GCC 兼容实现
- 新增 `SYSTEM/usart/syscalls_gcc.c`，为 newlib 提供最小系统调用封装

### 6. 说明

- 项目已完全转向 `Linux + OpenOCD` 工作流，不再维护 `Keil` 启用方式
