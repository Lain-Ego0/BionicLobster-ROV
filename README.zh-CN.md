...
- 双板控制架构：
  - `firmware`：主板，负责机械臂、尾部舵机、5 路 `AS5600`、`MS5837`、浮力步进电机、遥控串口以及向从板下发指令。
...
| 固件 | 作用 | 主要外设 |
| --- | --- | --- |
| `firmware` | 主控制板 | 2 个肘部 180 度舵机、2 个夹爪舵机、4 个手臂连续旋转舵机、2 个尾部舵机、5 路 `AS5600`、`MS5837`、`A4988`、`USART1` 板间串口、`USART2` 上位机 / 遥控串口 |
...
- 主板已经提供 5 路具名 `AS5600` 通道：
  - `left_arm_upper_360`
  - `left_arm_lower_360`
  - `right_arm_upper_360`
  - `right_arm_lower_360`
  - `tail_cable_joint`
- 从板当前实现了一个轻量级、基于 IMU 的四旋翼姿态保持混控。
